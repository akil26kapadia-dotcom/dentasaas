...
<div x-show="modalOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4"
    ...
    <div @click.outside="modalOpen = false"
        ...
        <h3 class="font-semibold text-lg text-gray-900 mb-4" x-text="editingId ? 'Edit Service' : 'Add Service'"></h3>

        <form method="POST"
            :action="editingId ? '{{ url('services') }}/' + editingId : '{{ route('services.store') }}'">
            @csrf
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <x-text-input id="service_name" name="name" class="block mt-1 w-full" x-model="form.name"
                        required />
                </div>
                <div>
                    <x-input-label for="service_name_hi" value="Name (Hindi)" />
                    <x-text-input id="service_name_hi" name="name_hi" class="block mt-1 w-full"
                        x-model="form.name_hi" />
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mt-4">
                <div>
                    ...
                    <div class="relative mt-1">
                        ...
                        <input id="service_price" name="price" type="number" min="0" step="0.01"
                            x-model="form.price" required
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 pl-7">
                    </div>
                </div>
                <div>
                    <x-input-label for="service_duration" value="Duration (min)" />
                    <x-text-input id="service_duration" name="duration_min" type="number" min="1"
                        class="block mt-1 w-full" x-model="form.duration_min" required />
                </div>
            </div>
                    ...
                    @foreach ($iconChoices as $icon)
                        <button type="button" @click="form.icon = '{{ $icon }}'"
                            ...
                            :class="form.icon === '{{ $icon }}' ?
                                'border-indigo-600 ring-2 ring-indigo-500 text-indigo-600' :
            ...
            <div class="flex items-center gap-2 mt-4">
                ...
                <label for="service_active" class="text-sm text-gray-600">Active</label>
            </div>
