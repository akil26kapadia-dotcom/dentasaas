<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="{
        ...
        @if ($services->isEmpty())
            <div class="rounded-2xl border border-gray-200 bg-white text-center py-16">
                <div
                ...
                @foreach ($services as $service)
                    <div class="rounded-2xl border border-gray-200 bg-white p-5">
                        <div class="flex items-start justify-between">
                            <span
                                class="w-14 h-14 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center text-2xl">
                            ...
                            <form method="POST" action="{{ route('services.toggle', $service) }}">
                                ...
                                <button type="submit"
                                    ...
                                    <span
                                        class="inline-block h-3.5 w-3.5 transform rounded-full bg-white transition {{ $service->is_active ? 'translate-x-4' : 'translate-x-1' }}"></span>
                                </button>
                            </form>
                        </div>

                        <p class="mt-4 font-medium text-gray-900">{{ $service->name }}</p>
                        @if ($service->name_hi)
                        ...
                        <div class="flex items-center justify-between mt-3">
                            <span class="text-lg font-semibold"
                                style="color:#465fff;">₹{{ number_format($service->price, 0) }}</span>
                            <span class="text-xs text-gray-500"><i class="fa-regular fa-clock"></i>
                                {{ $service->duration_min }} min</span>
                        </div>

                        <div class="flex items-center gap-4 mt-4 pt-4 border-t border-gray-100 text-sm">
                            <button type="button" @click="edit(@js(['id' => $service->id, 'name' => $service->name, 'name_hi' => $service->name_hi, 'price' => $service->price, 'duration_min' => $service->duration_min, 'description' => $service->description, 'icon' => $service->icon, 'is_active' => $service->is_active]))"
