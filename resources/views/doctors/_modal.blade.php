...
<div x-show="modalOpen" x-cloak
    ...
    <div @click.outside="modalOpen = false" class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md p-6">
        <h3 class="font-semibold text-lg text-gray-900 dark:text-white mb-4" x-text="editingId ? 'Edit Doctor' : 'Add Doctor'"></h3>
        ...
        <form method="POST" :action="editingId ? '{{ url('doctors') }}/' + editingId : '{{ route('doctors.store') }}'">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <x-text-input id="doctor_name" name="name" class="block mt-1 w-full" x-model="form.name" required />
                </div>

                <div>
                    <x-input-label for="doctor_email" value="Email" />
                    <x-text-input id="doctor_email" type="email" name="email" class="block mt-1 w-full" x-model="form.email" required />
                </div>
                ...
                <div>
                    ...
                    <x-text-input id="doctor_specialty" name="specialty" class="block mt-1 w-full" x-model="form.specialty" />
                </div>
                    ...
                    <div class="flex gap-2 mt-2">
                        @foreach ($swatches as $swatch)
                            <button type="button" @click="form.color = '{{ $swatch }}'"
                                    ...
                                    :class="form.color === '{{ $swatch }}' ? 'border-gray-800 dark:border-white' : 'border-transparent'"
            ...
            <div class="flex justify-end gap-3 mt-6">
                ...
                <button type="submit" class="px-4 py-2 text-sm rounded-lg bg-indigo-600 text-white hover:bg-indigo-700">Save Doctor</button>
            </div>
        </form>
    </div>
</div>
