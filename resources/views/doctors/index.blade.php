                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">{{ $doctor->specialty ?: 'General' }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between mt-4">
                            ...
                            <form method="POST" action="{{ route('doctors.toggle', $doctor) }}">
                                ...
                                <button type="submit"
                                    ...
                                    <span
                                        class="inline-block h-3.5 w-3.5 transform rounded-full bg-white transition {{ $doctor->is_active ? 'translate-x-4' : 'translate-x-1' }}"></span>
                                </button>
                            </form>
                        </div>

                        <div class="flex items-center gap-4 mt-4 pt-4 border-t border-gray-100 text-sm">
                            <button type="button" @click="edit(@js(['id' => $doctor->id, 'name' => $doctor->name, 'email' => $doctor->email, 'role' => $doctor->role, 'specialty' => $doctor->specialty, 'color' => $doctor->color]))"
