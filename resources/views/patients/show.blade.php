    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('patients.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Back to list</a>
            </div>
        </div>
    </x-slot>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6">
                <div class="flex flex-col items-center text-center">
                    <span
                        class="w-20 h-20 rounded-full flex items-center justify-center text-white text-2xl font-semibold"
                        style="background-color:#465fff;">
                        {{ strtoupper(substr($patient->name, 0, 1)) }}
                    </span>
                    <h3 class="mt-4 text-lg font-semibold text-gray-900">{{ $patient->name }}</h3>
                    @if ($patient->blood_group)
                        <span
                            class="mt-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-50 text-red-700">
                ...
                <dl class="mt-6 space-y-3 text-sm">
                    ...
                    <div class="flex justify-between">
                        ...
                        <dd class="text-gray-800">{{ $patient->gender ? ucfirst($patient->gender) : '—' }}</dd>
                    </div>
                    <div class="flex justify-between">
                        <dt class="text-gray-500">Date of Birth</dt>
                        <dd class="text-gray-800">{{ optional($patient->dob)->format('d M Y') ?: '—' }}</dd>
                    </div>
                    <div class="flex justify-between">
                        <dt class="text-gray-500">Added</dt>
                        <dd class="text-gray-800">{{ $patient->created_at->format('d M Y') }}</dd>
                    </div>
                </dl>

                @if ($patient->allergies)
                    <div class="mt-6">
                        <p class="text-xs font-medium text-gray-500 uppercase mb-2">Allergies</p>
                        <span
                ...
                @if ($patient->address)
                    <div class="mt-6">
                        <p class="text-xs font-medium text-gray-500 uppercase mb-2">Address</p>
            ...
            <div class="lg:col-span-2 rounded-2xl border border-gray-200 bg-white p-6">
                <div class="flex flex-wrap gap-1 border-b border-gray-100 mb-4">
                    @php
                        $tabs = [
                            'info' => 'Info',
                    ...
                    @foreach ($tabs as $key => $label)
                        <button @click="tab = '{{ $key }}'"
                            :class="tab === '{{ $key }}' ? 'border-indigo-600 text-indigo-600' :
                ...
                <div x-show="tab === 'info'">
                    <p class="text-sm text-gray-500">
                        Overview for {{ $patient->name }}. Use the tabs above to browse appointments, invoices,
                        prescriptions and treatment plans for this patient.
                    </p>
                </div>
                ...
                <div x-show="tab === 'appointments'" class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="py-6 text-center text-gray-400">No appointments yet.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div x-show="tab === 'invoices'" class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="py-6 text-center text-gray-400">No invoices yet.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div x-show="tab === 'prescriptions'" class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="py-6 text-center text-gray-400">No prescriptions yet.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                        ...
                        <tbody>
                            @forelse ($patient->treatmentPlans as $plan)
                                <tr class="border-t border-gray-100">
                                    ...
                                    <td class="py-2">{{ $plan->completed_count }}/{{ $plan->total_sessions }}
                                        ({{ $plan->progress_pct }}%)</td>
                                    <td class="py-2"><x-status-badge :status="$plan->status" /></td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="py-6 text-center text-gray-400">No treatment plans yet.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
