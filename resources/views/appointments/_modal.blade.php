<div x-show="modalOpen" x-cloak
    ...
    <div @click.outside="modalOpen = false" class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
        <h3 class="font-semibold text-lg text-gray-900 dark:text-white mb-4" x-text="editingId ? 'Edit Appointment' : 'New Appointment'"></h3>
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select class="block mt-1 w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-900 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <x-text-input id="appt_date" type="date" class="block mt-1 w-full" x-model="form.appt_date" required />
                </div>
                <div>
                    <x-input-label for="appt_time" value="Time" />
                    <x-text-input id="appt_time" type="time" class="block mt-1 w-full" x-model="form.appt_time" required />
                </div>
            </div>
        ...
        <div class="flex justify-end gap-3 mt-6">
            ...
            <button type="button" @click="submit()" class="px-4 py-2 text-sm rounded-lg bg-indigo-600 text-white hover:bg-indigo-700">Save Appointment</button>
        </div>
    </div>
</div>
