            ...
            <div class="inline-flex rounded-lg border border-gray-200 overflow-hidden">
                @foreach (['this_month' => 'This Month', 'last_3_months' => 'Last 3 Months', 'this_year' => 'This Year'] as $key => $label)
                    <a href="{{ route('analytics.index', ['period' => $key]) }}"
                        class="px-3 py-2 text-sm font-medium {{ $period === $key ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600' }}">
                        {{ $label }}
                    </a>
                @endforeach
            </div>
            ...
            @php
                $cards = [
                    ...
                    [
                        ...
                        'value' => number_format($stats['appointments_count']),
                    ],
                    [
                        'label' => 'Avg. Invoice Value',
                        'icon' => 'fa-file-invoice',
                        'value' => '₹' . number_format($stats['avg_invoice_value'], 2),
                    ],
                ];
            @endphp
        ...
        @if ($level === 'full')
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2 rounded-2xl border border-gray-200 bg-white p-6">
                    <h3 class="font-semibold text-gray-900 mb-4">Appointment Trend (Last 30 Days)</h3>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6">
                <h3 class="font-semibold text-gray-900 mb-4">Top Services</h3>
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div class="flex-1">
                                    <div class="flex justify-between text-sm mb-1">
                                        ...
                                        <span class="text-gray-500">{{ $service['count'] }} sold &bull;
                                            ₹{{ number_format($service['revenue'], 2) }}</span>
                                    </div>
                                    <div class="w-full h-1.5 rounded-full bg-gray-200">
                                        <div class="h-1.5 rounded-full"
                                            style="width: {{ ($service['revenue'] / $maxRevenue) * 100 }}%; background-color:#465fff;">
                                        </div>
                                    </div>
                                </div>
                            </div>
