<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">New Prescription</h2>
    </x-slot>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="prescriptionForm()" x-init="init()">
        ...
        <form method="POST" action="{{ route('prescriptions.store') }}" class="space-y-6">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="rx_date" type="date" name="rx_date" class="block mt-1 w-full"
                            value="{{ old('rx_date', now()->format('Y-m-d')) }}" required />
                    </div>

                    <div class="md:col-span-2">
                        <x-input-label for="diagnosis" value="Diagnosis" />
                        <textarea id="diagnosis" name="diagnosis" rows="2"
                            class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('diagnosis') }}</textarea>
                    </div>
                </div>
            </div>

            <div class="rounded-2xl border border-gray-200 bg-white p-6">
                <h3 class="font-semibold text-gray-900 mb-4">Medicines</h3>
                        ...
                        <tbody>
                            <template x-for="(med, index) in medicines" :key="index">
                                <tr class="border-t border-gray-100">
                                    ...
                                    <td class="py-2 pr-2">
                                        <input type="text" :name="`medicines[${index}][dose]`" x-model="med.dose"
                                            placeholder="500mg"
                                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    </td>
                                    <td class="py-2 pr-2">
                                    ...
                                    <td class="py-2 pr-2">
                                        <input type="text" :name="`medicines[${index}][duration]`"
                                            x-model="med.duration" placeholder="5 days"
                                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    </td>
                                    <td class="py-2 pr-2">
                                        <input type="text" :name="`medicines[${index}][instructions]`"
                                            x-model="med.instructions" placeholder="After food"
                                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    </td>
                                    <td class="py-2">
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('prescriptions.index') }}" class="px-4 py-2 text-sm text-gray-600">Cancel</a>
                <button type="submit"
