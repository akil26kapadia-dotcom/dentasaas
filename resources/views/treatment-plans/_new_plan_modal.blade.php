<div x-show="newPlanModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4"
    style="background-color: rgba(15,23,42,0.5);">
    <div @click.outside="newPlanModal = false"
        class="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
        <h3 class="font-semibold text-lg text-gray-900 mb-4">New Treatment Plan</h3>
        ...
        <form method="POST" action="{{ route('treatment-plans.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <x-text-input id="plan_patient_name" name="patient_name" class="block mt-1 w-full"
                    x-model="newPlanForm.patient_name" required />
            </div>
            ...
            <div>
                ...
                <x-text-input id="plan_treatment" name="treatment" class="block mt-1 w-full"
                    x-model="newPlanForm.treatment" placeholder="e.g. Root Canal" required />
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <x-input-label for="plan_total_sessions" value="Total Visits" />
                    <x-text-input id="plan_total_sessions" type="number" min="1" max="50"
                        name="total_sessions" class="block mt-1 w-full" x-model.number="newPlanForm.total_sessions"
                        required />
                </div>
                <div>
            ...
            <div class="flex justify-end gap-3 pt-2">
                <button type="button" @click="newPlanModal = false"
