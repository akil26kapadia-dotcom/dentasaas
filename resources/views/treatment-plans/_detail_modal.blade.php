<div x-show="detailModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4"
    style="background-color: rgba(15,23,42,0.5);">
    <div @click.outside="detailModal = false"
        class="bg-white rounded-xl shadow-xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
        ...
        <template x-if="detailPlan">
            <div>
                <div class="flex items-start justify-between mb-4">
                    <div>
                        <h3 class="font-semibold text-lg text-gray-900" x-text="detailPlan.patient_name"></h3>
                ...
                <div class="mb-6">
                    <div class="flex justify-between text-xs text-gray-500 mb-1">
                        <span
                            x-text="detailPlan.completed_count + ' / ' + detailPlan.total_sessions + ' visits completed'"></span>
                        <span x-text="detailPlan.progress_pct + '%'"></span>
                    </div>
                    <div class="w-full h-2.5 rounded-full bg-gray-200 overflow-hidden">
                        <div class="h-2.5 rounded-full bg-green-500 transition-all duration-500"
                            :style="`width: ${detailPlan.progress_pct}%`"></div>
                    </div>
                </div>
                        ...
                        <div class="border border-gray-100 rounded-lg p-4">
                            <div class="flex items-center justify-between mb-3">
                                <div class="flex items-center gap-2">
                                    <span
                                        class="w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold text-white"
                                        :class="{
                                    ...
                                    <span class="text-sm font-medium text-gray-800" x-text="session.title"></span>
                                </div>
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <a x-show="session.scheduled_date" x-cloak :href="session.whatsapp_url"
                                        target="_blank" rel="noopener" title="WhatsApp Reminder"
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                                <div>
                                    ...
                                    <input type="date"
                                        :value="session.scheduled_date ? session.scheduled_date.substring(0, 10) : ''"
                                        @change="saveSession(session, 'scheduled_date', $event.target.value)"
                                        class="block mt-1 w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                </div>
                                <div>
                                    <label class="text-xs text-gray-500">Time</label>
                                    <input type="time"
                                        :value="session.scheduled_time ? session.scheduled_time.substring(0, 5) : ''"
                                ...
                                <div>
                                    ...
                                    <select :value="session.status"
                                        @change="saveSession(session, 'status', $event.target.value)"
                                        class="block mt-1 w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
