    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-6">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                ...
                <p class="text-2xl font-semibold text-gray-900 mt-1">{{ $stats['count'] }}</p>
            </div>
        </div>
        ...
        <form method="GET" class="rounded-2xl border border-gray-200 bg-white p-4 mb-6 flex flex-wrap gap-3 items-end">
            <div class="flex-1 min-w-[200px]">
                ...
                <x-text-input id="q" name="q" class="block mt-1 w-full"
                    placeholder="Patient name or invoice no." :value="request('q')" />
            </div>
            <div>
            ...
            @if (request()->anyFilled(['q', 'status']))
                <a href="{{ route('invoices.index') }}" class="px-4 py-2 text-sm text-gray-500">Clear</a>
            @endif
        </form>
                        ...
                        <tbody>
                            @foreach ($invoices as $invoice)
                                <tr class="border-t border-gray-100">
                                    <td class="py-2">
                                        <a href="{{ route('invoices.show', $invoice) }}" class="font-bold"
                                            style="color:#465fff;">{{ $invoice->invoice_no }}</a>
                                    </td>
                                    <td class="py-2">{{ $invoice->patient_name }}</td>
                                    <td class="py-2">{{ $invoice->invoice_date->format('d M Y') }}</td>
                                    <td class="py-2">{{ count($invoice->items) }}
                                        item{{ count($invoice->items) === 1 ? '' : 's' }}</td>
