<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="invoiceForm()" x-init="init()">
        ...
        <form method="POST" action="{{ route('invoices.store') }}" class="space-y-6">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="patient_name" name="patient_name" class="block mt-1 w-full"
                            x-model="patient_name" required />
                        <x-input-error :messages="$errors->get('patient_name')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="invoice_date" value="Invoice Date" />
                        <x-text-input id="invoice_date" type="date" name="invoice_date" class="block mt-1 w-full"
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6">
                <h3 class="font-semibold text-gray-900 mb-4">Line Items</h3>
                            ...
                            <template x-for="(item, index) in items" :key="index">
                                <tr class="border-t border-gray-100">
                                        ...
                                        <select @change="onServiceSelect(index, $event.target.value)"
                                            ...
                                            <template x-for="s in services" :key="s.id">
                                                <option :value="s.id" x-text="s.name"
                                                    :selected="item.service_id == s.id"></option>
                                            </template>
                                        </select>
                                    ...
                                    <td class="py-2 pr-2 font-medium"
                                        x-text="'₹' + ((item.qty || 0) * (item.price || 0)).toFixed(2)"></td>
                                    <td class="py-2">
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-6 space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="tax_pct" type="number" min="0" max="100" step="0.01"
                                name="tax_pct" class="block mt-1 w-full" x-model.number="tax_pct" />
                        </div>
                        <div>
                            <x-input-label for="discount_pct" value="Discount %" />
                            <x-text-input id="discount_pct" type="number" min="0" max="100" step="0.01"
                                name="discount_pct" class="block mt-1 w-full" x-model.number="discount_pct" />
                        </div>
                    </div>
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-6">
                    ...
                    <dl class="space-y-2 text-sm">
                        <div class="flex justify-between">
                            ...
                            <dd class="text-gray-800" x-text="'₹' + subtotal().toFixed(2)"></dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-500">Discount (<span x-text="discount_pct || 0"></span>%)</dt>
                            <dd class="text-red-600" x-text="'- ₹' + discountAmount().toFixed(2)"></dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-500">GST (<span x-text="tax_pct || 0"></span>%)</dt>
                            <dd class="text-gray-800" x-text="'+ ₹' + taxAmount().toFixed(2)"></dd>
                        </div>
                    ...
                    <button type="submit"
                        class="w-full mt-6 inline-flex items-center justify-center px-4 py-2.5 rounded-lg font-medium text-white bg-indigo-600 hover:bg-indigo-700">
                        Save Invoice
                    </button>
                </div>
            </div>
