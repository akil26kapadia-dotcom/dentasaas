...
@if ($maxPct >= 80 && $resource)
    <div {{ $attributes->merge(['class' => 'flex items-center justify-between gap-4 rounded-lg border border-amber-200 bg-amber-50 dark:bg-amber-900/20 dark:border-amber-800 px-4 py-3 mb-4 text-sm']) }}>
        <div class="flex items-center gap-2 text-amber-800 dark:text-amber-300">
            <i class="fa-solid fa-triangle-exclamation"></i>
            <span>You've used {{ $usage[$resource]['pct'] }}% of your {{ str_replace('_', ' ', $resource) }} limit for this plan.</span>
        </div>
