<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6" x-data="{ patientModal: false, apptModal: false, invoiceModal: false }">
            ...
            @foreach ($cards as $card)
                <div class="rounded-2xl border border-gray-200 bg-white p-5 md:p-6">
                    ...
                    <div class="mt-5 flex items-end justify-between">
                        ...
                        <span
                            class="flex items-center gap-1 rounded-full py-0.5 pl-2 pr-2.5 text-sm font-medium {{ $card['trend'] >= 0 ? 'bg-success-50 text-success-600' : 'bg-error-50 text-error-600' }}">
        ...
        <div class="flex flex-wrap gap-3">
            <button @click="patientModal = true"
            ...
            @else
                <button disabled title="Coming soon"
                    class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-100 text-gray-400 text-sm font-medium cursor-not-allowed">
                    <i class="fa-solid fa-plus"></i> New Appointment
                </button>
            @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <div class="overflow-x-auto">
                    <table id="today-schedule-table" class="w-full text-sm">
                        ...
                        <tbody>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="py-6 text-center text-gray-400">No appointments scheduled
                                        for today.</td>
                                </tr>
                            @endforelse
                        </tbody>
            ...
            <div class="lg:col-span-2 rounded-2xl border border-gray-200 bg-white p-5">
                <h3 class="font-semibold text-gray-900 mb-4">Revenue (Last 6 Months)</h3>
            ...
            <ul class="space-y-3">
                @forelse ($recentActivity as $item)
                    <li class="flex items-center gap-3 text-sm">
                        ...
                        <span class="text-gray-700 flex-1">{{ $item['description'] }}</span>
                        <span
                            class="text-gray-400 text-xs whitespace-nowrap">{{ $item['created_at']->diffForHumans() }}</span>
                    </li>
                @empty
        ...
        <div x-show="patientModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4"
            style="background-color: rgba(15,23,42,0.5);">
            <div @click.outside="patientModal = false" class="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
                <h3 class="font-semibold text-lg text-gray-900 mb-4">New Patient</h3>
                <form method="POST" action="{{ route('patients.store') }}" class="space-y-4">
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                        <button type="button" @click="patientModal = false"
